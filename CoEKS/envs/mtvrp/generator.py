...
class MTVRPGenerator(Generator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(variant_preset, str) and variant_preset != "all" and variant_preset != "finetining":
    ...
    def subsample_problems(self, td):  # the set of tasks used for training
        batch_size = td.batch_size[0]

        combination_masks = [
            [False, False, False, False],  # 1. CVRP
            [True, False, False, False],  # 2. O
            [False, True, False, False],  # 3. TW
            [False, False, True, False],  # 4. L
            [False, False, False, True],  # 5. B
            [True, True, False, False],  # 6. OTW
            [True, False, True, False]  # 7. OL
        ]

        selected_indices = torch.randint(0, len(combination_masks), (batch_size,))
        keep_mask = torch.tensor([combination_masks[i] for i in selected_indices])
