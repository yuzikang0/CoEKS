...
class AttentionModelDecoder(AutoregressiveDecoder):
    ...
    def __init__(
        ...
        if pointer is None:
            ...
            pointer_attn_class = (
                PointerAttention if coe_kwargs is None else PointerAttnMoE
            )
    ...
    def _compute_q(self, cached: PrecomputedCache, td: TensorDict):
        ...
        assert not torch.isnan(node_embeds_cache).any(), "node_embeds_cache 包含 NaN"
        ...
        step_context, context_embedding = self.context_embedding(node_embeds_cache, td)

        assert not torch.isnan(step_context).any(), "step_context 包含 NaN"
        ...
        return glimpse_q, context_embedding
    ...
    def forward(
        ...
        glimpse_q, context_embedding = self._compute_q(cached, td)

        glimpse_k, glimpse_v, logit_k = self._compute_kvl(cached, td)

        # Compute logits
        mask = td["action_mask"]
        logits = self.pointer(glimpse_q, context_embedding, glimpse_k, glimpse_v, logit_k, active_experts, mask)
