...
from CoEKS.models.baselines.CoEKS.CoE import Task
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: Tensor, activate_index, mask: Optional[Tensor] = None) -> Tensor:
        if self.use_prenorm:
            # more modern transformer structure
            # https://arxiv.org/abs/2002.04745
            h = x + self.attention(self.norm_attn(x), mask)
            out_dense, coe_loss = self.ffn(self.norm_ffn(h), activate_index)
            h = h + out_dense
            ...
            h = self.norm_ffn(h + self.ffn(h), activate_index)
        return h
